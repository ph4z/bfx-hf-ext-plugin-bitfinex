...
const { TIME_FRAME_WIDTHS, preprocessRemoteCandles } = require('/home/duy/bfx-hf-util')
...
module.exports = async (candleModel, {
  ...
  return PI.mapSeries(consolidatedBounds, async ({ start, end }) => (
    ...
    return PI.forEachSeries(candleSets, async (raw_candles) => {
      const candles =  raw_candles.map(x => new Candle(x, this) )   
